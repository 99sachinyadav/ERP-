...
const registerTeacher = async (req,res)=>{
    ...
    try {
        ...
        // if(!SectionExist) {
        //     return res.status(401).json({sucess:false,message:"section is not available"})
        // }
...
//     await newsection.save();
    ...
    return res.status(201).json({ success: true, message: 'Teacher saved successfully ', teacherToken });
...
const teacherLogin = async (req , res)=>{
    try {
        ...
        if(!email ||!password){
            return res.status(403).json({sucess:false,message:"email or password is missing"})
        }
        ...
        if (!findTeacher) {
            return res.status(401).json({ sucess: false, message: "faculty does not exist" });
        }
         const ispasswordMatched = await bcrypt.compare(password,findTeacher.password)
        ...
        return res.status(200).json({ success: true, message: 'Login successful', refeshTeacherToken,  teacher: {
            id: findTeacher._id,
...
const adminLogin = async (req,res)=>{
    try {
        const {email , password} = req.body;
        if(!email || !password){
            return res.status(403).json({sucess:false,message:"please enter full details"})
        }
        if(email===process.env.ADMIN_EMAIL && password === process.env.ADMIN_PASSWORD){
              const adminToken = generateToken(process.env.ADMIN_EMAIL+process.env.ADMIN_PASSWORD)
              return res.status(200).json({sucess:true,message:'admin login sucessfully',adminToken})
        }
        else{
            return res.status(401).json({sucess:false,message:"Invalid admin credentials"})
        }
    } catch (error) {
          console.log(error)
 ...
 const attendancebyTeacher = async (req,res)=>{
    try {
        const {subject, rollno, date, noofLecAttended , totalnoLec,teacherId} = req.body;
        if(!subject ||!rollno || !date || !noofLecAttended || !totalnoLec){
            return res.status(403).json({sucess:false,message:"kindly fill you full details"})
        }
        const findStudent = await Student.findOne({rollno:rollno});
        if(!findStudent){
            return res.status(401).json({sucess:false,message:"student not found"})
        }
        const findTeacherbyAuth = await Teacher.findOne({_id:teacherId});
        if(!findTeacherbyAuth){
            return res.status(401).json({sucess:false,message:"Teacher not found in database"})
        }
        const sectionName  = findStudent.section+findStudent.year+"_"+findStudent.batch;
          const findSection = await Section.findOne({name:sectionName});
        if(!findSection){
            return res.status(401).json({sucess:false,message:"section assigned to student not found"}) }
        if(findSection.teacher.toString() !== findTeacherbyAuth._id.toString()){
            return res.status(401).json({sucess:false,message:"faculty not assigned to this section"})
        }
        const dateObject = new Date(date)
        findStudent.attendance.push({subject:subject,date:dateObject,noofLecAttended:noofLecAttended,totalnoLec:totalnoLec})
        await findStudent.save();   
        return res.status(200).json({sucess:true,message:"attendance updated successfully"})    
 ...
 const updateTeacherInSection = async (req , res)=>{
    try {
        ...
        if(!ExistedTeacher){
            return res.status(401).json({sucess:false,message:"faculty not exist"})

        }
         const SectionName = section+year+"_"+batch;
        const SectionExist = await Section.findOne({ name: SectionName , year:year , batch:batch});
        if(!SectionExist) {
            return res.status(401).json({sucess:false,message:"section is not available"})
        }
         const previousTeacher = await Teacher.findById(SectionExist.teacher);
          previousTeacher.section = previousTeacher.section.filter((sectionId) => sectionId.toString() !== SectionExist._id.toString());
        ...
        return res.status(200).json({sucess:true,message:"teacher updated in section successfully"})
    } catch (error) {
        console.log(error)
        return res.status(401).json({sucess:false,message:error.message})
    }
 ...
 const getAllTeacher = async (req,res)=>{
    try {
        const findAllTeacher = await Teacher.find({}).select('-password').populate('section');
        if(!findAllTeacher){
            return res.status(401).json({sucess:false,message:"no teacher is registered now"})
        }
        return res.status(200).json({sucess:true,message:"all teacher fetched sucessfully",findAllTeacher})
...
// by this route we can get all the attendance of all student in a particular section with student data
 const getAttendanceofAllStudent = async (req,res)=>{
    try {
        ...
        return res.status(200).json({sucess:true,message:"all student attendance fetched successfully",SectionStudent})
...
export { registerTeacher ,teacherLogin,adminLogin,attendancebyTeacher,updateTeacherInSection,getAllTeacher,getAttendanceofAllStudent};
