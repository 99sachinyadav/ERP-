...
const ChangeStaffpassword = () => {
  ...
  const handleSubmit = async (e) => {
     ...
     try {
       const responce = await axios.put(backendUrl + '/api/updateStaffPassword',{
       email: staffemail,
       newPassword
       },{
        headers:{
   ...
   <div className='flex  justify-center items-center h-screen bg-gray-100 relative'>
        <div className="flex flex-col   pb-20 sm:pb-50 items-center gap-5  bg-gray-100 ">
         <h1 className=" text-2xl sm:text-4xl    flex justify-center sm:mt-10  font-bold  text-blue-900   text-wrap ">
          Change Staff<span className="text-red-500 ml-3">Password</span>
          </h1> 
        ...
        <form onSubmit={handleSubmit} className="flex flex-col gap-6 w-84 sm:w-[450px] mt-10 bg-white p-5 sm:p-8 rounded-lg shadow-md">
          ...
          <input  value={staffemail} onChange={(e)=>setemail(e.target.value)}  placeholder='Enter Your Email'   type="email" id="section" className="border border-gray-300 rounded-md py-2  p-1" required />
