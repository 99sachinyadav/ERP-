 ...
 const  SpecialstudentMove = () => {
     ...
     const changeSection = async (studentId ) => {
            ...
            console.log(year,batch,section,Remark[studentId], studentId, name ,rollno);
            
            try {
 
             const response = await axios.post(backendUrl + '/api/addSpecialStudent',{
                 year,
                 batch,
                 section,
                 name: student?.name ?? name,
                 rollno: student?.rollno ?? rollno,
                 Remarks:Remark[studentId],
                 ...
                 <thead className="border border-gray-300 bg-gray-200 w-full">
                     <tr className="bg-gray-100">
                         ...
                         <th className="p-1 sm:p-3 border text-left min-w-[80px] sm:min-w-[120px] text-xs sm:text-base">New Year</th> */}
                              <th className="p-1 sm:p-3 border text-left min-w-[80px] sm:min-w-[120px] text-xs sm:text-base">Remark</th>
                         ...
                         <tr key={idx} className={`${idx % 2 === 0 ? "bg-white" : "bg-gray-50"}`}>
                             ...
                             <td className="p-1 sm:p-3 border w-1/4 min-w-[50px] sm:max-w-[50px] text-xs sm:text-base">{student.section}</td>
                               <td className="p-1 sm:p-3 border w-1/4 min-w-[80px] sm:min-w-[120px] text-xs sm:text-base">
                                 <input
                                     type="text"
                                     value={Remark[student._id]??''}
                                     onChange={(e) => setRemark(prev=>({...prev , [student._id]: e.target.value??''}))}
 ...
 export default SpecialstudentMove