...
const getStudent = async ()=>{
  ...
  return (
      <div className={`flex flex-col  `} >
            <div className={`grid grid-cols-4 items-center justify-items-center border border-gray-300 gap-4 p-4 bg-white ${popupStudent?" bg-black opacity-40 ":""} rounded-lg shadow-md`}>
             <input
             value={section}
         ...
         <thead>
           <tr>
             ...
             <th className="border border-gray-300   text-xs sm:text-lg sm:px-4 py-2 bg-gray-100 text-left">Attendance</th>
            ...
            return (
                <>
                    <tr
                        ...
                        key={idx}
                        className={`${((totalAttend / (totalLec )) * 100).toFixed(2) < 75 ? "bg-red-400" : "even:bg-gray-50"} ${popupStudent && popupStudent? "opacity-40" : ""}`}
                    ...
                    >
                        ...
                        <td className="border border-gray-300 text-xs sm:text-lg sm:px-4 py-2">
                            {totalLec > 0 ? `${((totalAttend / totalLec) * 100).toFixed(2)}%` : "N/A"}
                        </td>
                        <td className="border border-gray-300 text-xs text-wrap sm:text-lg text-center sm:px-4 py-2">{student?.father_name}</td>
                        <td className="border border-gray-300 whitespace-normal max-w-[120px] text-xs">{student?.rollno}</td>
                    </tr>
                            ...
                            <td colSpan={4}>
                                <div className="fixed inset-0 flex items-center justify-center bg-transparent bg-opacity-40 z-50">
                                    <div className="bg-white p-6 rounded shadow-lg min-w-[300px] max-w-[90vw]">
                                        <h2 className="text-lg font-bold mb-2">Student Attendance Details</h2>
                                        <p><strong>Name:</strong> {popupStudent.name}</p>
                                        <p><strong>Roll No:</strong> {popupStudent.rollno}</p>
                                        <p><strong>Father's Name:</strong> {popupStudent.father_name}</p>
                                        <h3 className="font-semibold mt-4 mb-2">Subject-wise Attendance</h3>
                                        {popupStudent.attendance && popupStudent.attendance.length > 0 ? (
                                            popupStudent.attendance.map((attend, i) => {
                                              let totalLec = 0;
                                              let totalAttend = 0;
                                              let subjectName =""
                                              {attend.subject && attend.subject.forEach((subj)=>{
                                                  totalLec += subj.totalnoLec || 0;
                                                  totalAttend += subj.noofLecAttended || 0;
                                                  subjectName = subj.name || "";
                                              })}
                                           return    ( <div key={i} className="mb-2">
                                                    {Array.isArray(attend.subject) && attend.subject.length > 0 ? (
                                                        <table className="w-full text-xs mb-2">
                                                            <thead>
                                                                <tr>
                                                                    <th className="border px-2 py-1">Subject</th>
                                                                    <th className="border px-2 py-1">Total Lectures</th>
                                                                    <th className="border px-2 py-1">Lectures Attended</th>
                                                                    <th className="border px-2 py-1">Attendance %</th>
                                                                </tr>
                                                            </thead>
                                                            <tbody>
                                                                <tr key={i} className={`${((totalAttend / totalLec) * 100).toFixed(2) < 75 ? "bg-red-400" : "even:bg-gray-50"}`}>
                                                                    <td className="border px-2 py-1">{subjectName|| "N/A"}</td>
                                                                    <td className="border px-2 py-1">{totalLec || 0}</td>
                                                                    <td className="border px-2 py-1">{totalAttend || 0}</td>
