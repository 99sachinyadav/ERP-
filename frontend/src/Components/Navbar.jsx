...
const Navbar = () => {
  ...
  return (
    <div className="w-[370px] sm:w-full h-[90px] sm:h-[110px]  flex start-0 shadow-md relative ">
      <img
      ...
      <div className="h-full    font-[700] text-blue-900 w-[70%] ml-10 flex gap-20 justify-center items-center border-amber-600">
        <p className="hidden sm:block   text-2xl">HOME</p>
        <p className="hidden sm:block text-2xl">PROFILE</p>
        <p className="hidden sm:block text-2xl">ABOUT</p>
        <p className="hidden sm:block text-2xl">DEPARTMENT</p>
        <p className="hidden sm:block text-2xl">CONTACT</p>
      </div>

    <div className="  relative ml-[-110px] cursor-pointer sm:ml-10 sm:mt-8 mt-10 ">
    
    <i  onClick={()=>{setuser(true)}}  className="ri-user-line absolute top-0 left-4.5  sm:top-[-30px]  text-black sm:pr-[250px] sm:ml-10 sm:mt-8   cursor-pointer  text-2xl  sm:text-4xl"></i>
      ...
      <div
        className={`absolute top-0 right-0 bottom-0 overflow-hidden h-[739px]  z-40 bg-white ${
          Isopen ? "w-full" : "w-0 "
        }`}
      >
