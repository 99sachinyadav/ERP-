...
const Navbar = () => {
  ...
  return (
    <div className="w-full sm:w-full h-[90px] sm:h-[90px] flex start-0 shadow-md relative">
      ...
      <div className="h-full font-[700] text-blue-900 w-[70%] ml-10 flex gap-4 sm:gap-15 justify-center items-center border-amber-600">
        <Link to="/home" className="hidden sm:block text-sm sm:text-2xl">
          HOME
        </Link>
        <Link to="/profile" className="hidden sm:block text-sm sm:text-2xl">
          PROFILE
        </Link>
      ...
      <div className="relative sm:hidden lg:block  ml-[-110px] cursor-pointer sm:ml-10 sm:mt-8 mt-10">
        <i
          ...
          className="ri-user-line absolute top-0 left-4.5 sm:left-[-170px] sm:top-[-30px] text-black sm:pr-[250px] sm:ml-10 sm:mt-8 cursor-pointer text-2xl sm:text-4xl"
        ></i>

        <div
          className={`${
            user ? "block" : "hidden"
          } absolute dropdown-menu right-[0px] sm:right-[120px] pt-4`}
        >
          <div className="flex flex-col bg-slate-100 w-36 gap-2 text-gray-500 rounded py-3 px-5">
            <p onClick={() => navigator("/profile")} className="cursor-pointer hover:text-black">My Profile</p>
            <p
              onClick={() => {
                navigator("/"), setuser(false);
              }}
      ...
      >
        <div className="flex flex-col w-full">
          ...
          <div
            onClick={() => {
              navigator("/home"), setIsopen(false), setcolorbg("home");
            }}
            className={`flex gap-8 h-12 ${
              colorbg == "home" ? "bg-pink-300 text-blue-900" : "bg-white"
            } pl-3 items-center`}
          >
          ...
          <div
            onClick={() => {
              navigator("/profile"), setIsopen(false), setcolorbg("profile");
            }}
            className={`flex gap-8 h-12 ${
              colorbg == "profile" ? "bg-pink-300 text-blue-900" : "bg-white"
            } pl-3 items-center`}
          >
          ...
          <div
            onClick={() => {
              navigator("/about"), setIsopen(false), setcolorbg("about");
            }}
            className={`flex gap-8 h-12 ${
              colorbg == "about" ? "bg-pink-300 text-blue-900" : "bg-white"
            } pl-3 items-center`}
          >
          ...
          <div
            onClick={() => {
              navigator("/contact"), setIsopen(false), setcolorbg("contact");
            }}
            className={`flex gap-8 h-12 ${
              colorbg == "contact" ? "bg-pink-300 text-blue-900" : "bg-white"
            } pl-3 items-center`}
          >
